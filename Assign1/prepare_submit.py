# -*- coding: utf-8 -*-
# @Author: Haiqin Yang
# @Date:   2025-10-07 22:34:34
# @Last Modified by:   Haiqin Yang
# @Last Modified time: 2025-10-07 22:55:13
#
...
required_files = {'Assign1.py', 'Assign1_func.py', 'out.md', 'util.py'}

def check_file(file: str, check_aid: str):
    file_name = os.path.basename(file)
    assert file_name == f"{check_aid}-Assign1.zip", f"Zip file name should be {check_aid}-Assign1.zip"
    ...
    with zipfile.ZipFile(file, 'r') as zz:
        ...
        for info in zz.infolist():
            ...
            assert info.filename.startswith(target_prefix), \
                'There should only be one top-level dir (with your 12-digit campus id as the dir-name) inside the zip file.'
    ...
    print(f"Read zipfile {file}, please check that your 12-digit campus id is: {target_prefix[:-1]}")
...
def main(path: str, aid: str):
    ...
    if os.path.isdir(path):
        with zipfile.ZipFile(f"{aid}-Assign1.zip", 'w') as zz:
            for root, dirs, files in os.walk(path):
                if '.git' in root or '__pycache__' in root:
                    continue  # ignore some parts
                for file in files:
                    if any(file.endswith(s) for s in ['.md', '.py', '.npy']):
        ...
        check_file(f'{aid}-Assign1.zip', aid)
    else:  # directly check
        check_file(path, aid)
    # --
